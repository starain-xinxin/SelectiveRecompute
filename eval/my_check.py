...
def check_backward_validity(inputs: Iterable[Any]) -> None:
    """
    对于输入的参数，至少有一个是含有梯度的(不然反向传播时找不到backward的点)
    :param inputs: 一个可迭代对象，这里是被封装的function的前向的参数
    """
...
def _get_device_module(device="cuda"):
    """
    获取指定设备的设备模块，有点像上下文模块 TODO：不确定
    python:
    >>> device_module = getattr(torch, 'cuda')
    >>> <module 'torch.cuda' from 'balabala/anaconda3/envs/***/lib/python3.9/site-packages/torch/cuda/__init__.py'>
     """
...
class DefaultDeviceType:
    ...
    如果不存在非 CPU 张量，则将使用默认设备类型。默认值为 'cuda'。
    在检查点过程中，设备类型用于确定要保存和恢复的设备状态，以便进行重新计算。
    """
...
def _infer_device_type(*args):
    """ 获取设备标识字符串 """
    device_types = list(
        {   # 集合推导式：对于是tensor且不在cpu上的参数，将他们的设备放入列表
            arg.device.type
            for arg in args
    ...
    if len(device_types) == 0:
        # 没有非cpu设备，就选取默认设备
        return DefaultDeviceType.get_device_type()  # 这里实际上是默认'cuda'
    elif "cuda" in device_types:
        # 有cuda设备优先cuda设备
        return "cuda"
    else:
        # 如果有多种非cuda与cpu设备，选第一个设备
...
def get_device_states(*args) -> Tuple[List[int], List[torch.Tensor]]:
    """ 获得这些非cpu设备的随机数状态
    :return fwd_device_states
    :return fwd_device_ids    前向的机器的id
    """
    fwd_device_ids = list(  # 对所有输入张量遍历一遍他们的设备，把非cpu的设备放到这个列表里
        {
            arg.get_device()
            for arg in args
            if isinstance(arg, torch.Tensor) and not arg.device.type == "cpu"
    ...
    for device_id in fwd_device_ids:
        # TODO: 这里是关于获得设备的上下文等信息的代码，需要进一步了解pytorch的架构
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, preserve_rng_state, *args):
        """
        :param ctx.fwd_cpu_state:一个张量，对应cpu的随机数状态
        :param ctx.had_device_in_fwd:非cpu设备是否存在于前向计算中
        :param ctx:
        :param run_function: 封装的function
        :param preserve_rng_state: 是否保留随机数状态
        :param args: function的forward输入参数
        :return:
        """
        check_backward_validity(args)       # 检查输入张量至少有一个有梯度
        ctx.run_function = run_function     # 被封装的function
        ctx.preserve_rng_state = preserve_rng_state     # 是否保留随机数状态

        ctx.device = _infer_device_type(*args)  # 获得当前的非cpu设备
        ctx.device_autocast_kwargs, ctx.cpu_autocast_kwargs = _get_autocast_kwargs(
            ctx.device
        )       # 获取自动混合进度的参数
        if preserve_rng_state:
            ctx.fwd_cpu_state = torch.get_rng_state()     # 一个张量，对应cpu的随机数状态
            # 不要意外地过早初始化 CUDA 上下文。
            # 如果用户打算稍后在他们的 run_function 中初始化上下文，我们确实应该在这里存储 CUDA 状态。
            # 不幸的是，我们无法预料在运行函数之前会发生这种情况
            ctx.had_device_in_fwd = False
            device_module = _get_device_module(ctx.device)  # 获得设备的device_module
            if getattr(device_module, "_initialized", False):   # 看设备如果被初始化过
                ctx.had_device_in_fwd = True    # 在前向计算中存在这个device
                ctx.fwd_devices, ctx.fwd_device_states = get_device_states(*args)       # 获得非cpu设备的随机数状态

        # Save non-tensor inputs in ctx, keep a placeholder None for tensors
        # to be filled out during the backward.
        # 将非张量输入保存在上下文中，在反向传播期间保留张量的占位符 None，待填充。
        ctx.inputs = []
        ctx.tensor_indices = []
        tensor_inputs = []
        for i, arg in enumerate(args):
            ...
            if torch.is_tensor(arg):
                # tensor_inputs 这个列表是存核心张量的，由于害怕张量被 in-place修改，所以后续使用ctx.save_for_backward()存储
                tensor_inputs.append(arg)       # 将张量写入 tensor_inputs
                ctx.tensor_indices.append(i)    # 将张量是输入的id（第几个输入参数）写入  ctx.tensor_indices
                ctx.inputs.append(None)         # 用none来在inputs中帮张量占位
            else:
                ctx.inputs.append(arg)          # 非张量输入，直接写进 ctx.inputs

        # `saved_for_backward`是会保留此input的全部信息, 并避免in-place操作导致的input在backward被修改的情况.
        # 它是将函数的输入参数保存起来以便后面在求导时候再使用，起前向反向传播中协调作用。
        ctx.save_for_backward(*tensor_inputs)

        with torch.no_grad():       # no_grad()模式前向计算
            outputs = run_function(*args)
        # 这里是无梯度的输出，但checkpoint函数调用CheckpointFunction.apply(f)时，
        # 并没有with no_grad(),所以这个Function的输出最终是 requires_grad=True
        # 输出的 grad_fn = <'CheckpointFunctionBackward'>
        # 而反向传播时，利用这一点找到需要的backward函数
        return outputs
...
def checkpoint(
    ...
    **kwargs
    ...
    if use_reentrant:
        ...
        a = CheckpointFunction.apply(function, preserve, *args)
        return a
    else:
